...
Le fichier `config.yaml` contient toute la configuration :
...
Le modèle IsolationForest peut être entraîné automatiquement si `use_ml: true` dans la configuration et qu'un fichier baseline existe.
...
```bash
python tools/pcap_generator.py
```
